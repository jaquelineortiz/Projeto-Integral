...
function test()
  @testset "Testes manuais" begin
    for (f, a, b, c, d, I, nome) in [((x,y) -> exp(-x^2 - y^2), -30, 30, -30, 30, π, "exp-dupla"),
                                     ...
                                     ((x,y) -> (2 * y * sin(x) + (cos(x))^2), 0, π/4, 0, π/2, 1.73223, "trigonometrica"),
                                     ((x,y) -> x / (sqrt(1 - y^2)), 0, 1, 0, 1, 0,785398, "sobre raiz"),
                                     ((x,y) -> (y * sin(x) + x * cos(y)), -π, 3*π/2, 0, 2*π, -19.7392, "trigonometrica2"),
